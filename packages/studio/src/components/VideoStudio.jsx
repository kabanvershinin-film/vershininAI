...
import {
  studioVisibleT2VModels as t2vModels,
  studioVisibleI2VModels as i2vModels,
  studioVisibleV2VModels as v2vModels,
...
const VIDEO_MODEL_PARAM_SKIP_KEYS = [
  "prompt",
  "aspect_ratio",
  "duration",
  "resolution",
...
const VIDEO_PERSIST_SCHEMA_VERSION = 2;
const LEGACY_DEFAULT_T2V_MODEL_IDS = new Set(["seedance-v1.5-pro-t2v"]);
...
const VideoBrandFallbackIcon = ({ className }) => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
  ...
  >
    ...
    <path d="M16 8l4-2.5v9L16 12" />
    <path d="M8 19h8" />
  </svg>
);

const VIDEO_MODEL_BRANDS = [
  { key: "seedance", label: "Seedance", Icon: SiBytedance, className: "bg-rose-500/15 text-rose-300 border-rose-400/20", match: (id) => id.includes("seedance") || id.includes("doubao-seedance") },
  { key: "kling", label: "Kling", Icon: SiKuaishou, className: "bg-lime-500/15 text-lime-300 border-lime-400/20", match: (id) => id.includes("kling") },
  { key: "veo", label: "Veo", Icon: SiGoogle, className: "bg-blue-500/15 text-blue-300 border-blue-400/20", match: (id) => id.includes("veo") },
  { key: "wan", label: "Wan", Icon: SiAlibabacloud, className: "bg-cyan-500/15 text-cyan-300 border-cyan-400/20", match: (id) => id.includes("wan") },
  { key: "pixverse", label: "PixVerse", Icon: SiPix, className: "bg-fuchsia-500/15 text-fuchsia-300 border-fuchsia-400/20", match: (id) => id.includes("pixverse") },
  { key: "hailuo", label: "Hailuo", Icon: VideoBrandFallbackIcon, className: "bg-orange-500/15 text-orange-300 border-orange-400/20", match: (id) => id.includes("hailuo") },
  { key: "sora", label: "Sora", Icon: SiOpenai, className: "bg-emerald-500/15 text-emerald-300 border-emerald-400/20", match: (id) => id.includes("sora") || id.includes("openai-sora") },
  { key: "vidu", label: "Vidu", Icon: VideoBrandFallbackIcon, className: "bg-violet-500/15 text-violet-300 border-violet-400/20", match: (id) => id.includes("vidu") },
  { key: "ovi", label: "OVI", Icon: VideoBrandFallbackIcon, className: "bg-teal-500/15 text-teal-300 border-teal-400/20", match: (id) => id.includes("ovi") },
  { key: "gemini", label: "Gemini", Icon: SiGooglegemini, className: "bg-amber-500/15 text-amber-300 border-amber-400/20", match: (id) => id.includes("omni") || id.includes("gemini") },
  { key: "happyhorse", label: "HappyHorse", Icon: VideoBrandFallbackIcon, className: "bg-yellow-500/15 text-yellow-300 border-yellow-400/20", match: (id) => id.includes("happyhorse") },
  { key: "grok", label: "Grok", Icon: SiX, className: "bg-neutral-400/15 text-neutral-200 border-neutral-300/20", match: (id) => id.includes("grok") },
  { key: "ltx", label: "LTX", Icon: VideoBrandFallbackIcon, className: "bg-indigo-500/15 text-indigo-300 border-indigo-400/20", match: (id) => id.includes("ltx") },
  { key: "luma", label: "Luma", Icon: VideoBrandFallbackIcon, className: "bg-sky-500/15 text-sky-300 border-sky-400/20", match: (id) => id.includes("luma") },
  { key: "runway", label: "Runway", Icon: VideoBrandFallbackIcon, className: "bg-purple-500/15 text-purple-300 border-purple-400/20", match: (id) => id.includes("runway") },
  { key: "hunyuan", label: "Hunyuan", Icon: VideoBrandFallbackIcon, className: "bg-red-500/15 text-red-300 border-red-400/20", match: (id) => id.includes("hunyuan") },
  { key: "minimax", label: "MiniMax", Icon: VideoBrandFallbackIcon, className: "bg-orange-500/15 text-orange-300 border-orange-400/20", match: (id) => id.includes("minimax") },
  { key: "midjourney", label: "Midjourney", Icon: VideoBrandFallbackIcon, className: "bg-pink-500/15 text-pink-300 border-pink-400/20", match: (id) => id.includes("midjourney") },
  { key: "leonardo", label: "Leonardo", Icon: VideoBrandFallbackIcon, className: "bg-green-500/15 text-green-300 border-green-400/20", match: (id) => id.includes("leonardo") },
  { key: "video-tools", label: "Video Tools", Icon: VideoBrandFallbackIcon, className: "bg-orange-500/15 text-orange-300 border-orange-400/20", match: (id) => id.includes("watermark") || id.includes("effects") || id.includes("motion") || id === "vfx" },
];

const OTHER_VIDEO_BRAND = {
  key: "other",
  label: "Other",
  Icon: VideoBrandFallbackIcon,
  className: "bg-primary/10 text-primary border-white/5",
};

function getVideoModelBrand(model) {
  const id = (model?.id || "").toLowerCase();
  return VIDEO_MODEL_BRANDS.find((brand) => brand.match(id)) || OTHER_VIDEO_BRAND;
}

function groupVideoModelsByBrand(models) {
  const groups = [];
  const groupByKey = new Map();

  models.forEach((model) => {
    const brand = getVideoModelBrand(model);
...
function VideoBrandLogo({ brand, size = "large" }) {
  const Icon = brand.Icon || VideoBrandFallbackIcon;
  const sizeClass = size === "large" ? "w-10 h-10 rounded-xl" : "w-5 h-5 rounded";
  const iconClass = size === "large" ? "w-4.5 h-4.5" : "w-3 h-3";

  return (
    <div
      className={`${sizeClass} ${brand.className} border flex items-center justify-center shadow-inner`}
  ...
  const renderItem = (m, isV2V = false) => (
    ...
    >
      <div className="flex items-center gap-3.5">
        <VideoBrandLogo brand={getVideoModelBrand(m)} />
  ...
  return (
    <div className="flex flex-col h-full max-h-[70vh]">
      ...
      <div className="flex flex-col gap-1.5 overflow-y-auto custom-scrollbar pr-1 pb-2">
        {groupVideoModelsByBrand(filteredMain).map(({ brand, models }) => (
          <div key={brand.key} className="flex flex-col gap-1.5">
            <div className="px-3 pt-2 pb-1 text-[10px] font-bold uppercase tracking-[0.12em] text-muted">
              {brand.label}
            </div>
            {models.map((m) => renderItem(m, false))}
          </div>
        ))}
        {filteredV2V.length > 0 && (
          <>
            ...
            {groupVideoModelsByBrand(filteredV2V).map(({ brand, models }) => (
              <div key={`v2v-${brand.key}`} className="flex flex-col gap-1.5">
                <div className="px-3 pt-2 pb-1 text-[10px] font-bold uppercase tracking-[0.12em] text-muted">
                  {brand.label}
                </div>
                {models.map((m) => renderItem(m, true))}
              </div>
            ))}
          </>
        )}
...
export default function VideoStudio({
  ...
  const [modelParams, setModelParams] = useState(() =>
    createInitialModelParams(defaultModel, {}, VIDEO_MODEL_PARAM_SKIP_KEYS),
  );
  ...
  const applyControlsForModel = useCallback(
    (modelId, isImageMode, isV2vMode) => {
      if (isV2vMode) {
        ...
        setModelParams((prev) =>
          createInitialModelParams(model, prev, VIDEO_MODEL_PARAM_SKIP_KEYS),
        );
        setShowAr(false);
  ...
  useEffect(() => {
    ...
    if (!v2vMode) setPromptDisabled(false);
  }, [getCurrentModel, getCurrentModels, imageMode, v2vMode, applyControlsForModel]);
    ...
    try {
      ...
      if (stored) {
        ...
        const shouldMigrateLegacyDefault =
          (!data.persistSchemaVersion || data.persistSchemaVersion < VIDEO_PERSIST_SCHEMA_VERSION) &&
          !data.imageMode &&
          !data.v2vMode &&
          LEGACY_DEFAULT_T2V_MODEL_IDS.has(data.selectedModel);
        const restoredModelId = shouldMigrateLegacyDefault
          ? defaultModel.id
          : data.selectedModel;
        const restoredModel = getCurrentModels().find((model) => model.id === restoredModelId) || defaultModel;
        if (restoredModelId) setSelectedModel(restoredModel.id);
        if (shouldMigrateLegacyDefault || data.selectedModelName) {
          setSelectedModelName(shouldMigrateLegacyDefault ? defaultModel.name : data.selectedModelName);
        }
        ...
        applyControlsForModel(
          restoredModel.id,
          shouldMigrateLegacyDefault ? false : !!data.imageMode,
          shouldMigrateLegacyDefault ? false : !!data.v2vMode
        );
      }
    } catch (err) {
  ...
  }, [applyControlsForModel, defaultModel, getCurrentModels]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        const state = {
          persistSchemaVersion: VIDEO_PERSIST_SCHEMA_VERSION,
          imageMode,
          v2vMode,
          selectedModel,
          selectedModelName,
    ...
    (m, isV2V) => {
      if (isV2V) {
        ...
        if (isMC) {
          ...
          setPromptDisabled(false);
        } else if (m.hasPrompt || m.promptRequired) {
          setPromptDisabled(false);
        } else {
          setPrompt("");
          setPromptDisabled(true);
    ...
    try {
      ...
      if (v2vMode) {
        ...
        const v2vParams = {
          ...
          videoRefs: [uploadedVideoUrl],
        };
        if (currentModel?.imageField && uploadedImageUrl) {
          v2vParams.image_url = uploadedImageUrl;
          v2vParams.imageWithRoles = [{ role: "first_frame", url: uploadedImageUrl }];
          v2vParams.referenceImages = [uploadedImageUrl];
        }
        ...
        const i2vParams = {
          ...
          imageWithRoles: [{ role: "first_frame", url: uploadedImageUrl }],
        ...
        if (uploadedEndImageUrl && i2vModel?.lastImageField) {
          ...
          i2vParams.imageWithRoles = [
            { role: "first_frame", url: uploadedImageUrl },
            { role: "last_frame", url: uploadedEndImageUrl },
          ];
          i2vParams.referenceImages = [uploadedImageUrl, uploadedEndImageUrl];
        }
        ...
        const params = { ...sanitizedModelParams, model: selectedModel };
        if (trimmedPrompt) params.prompt = trimmedPrompt;
        if (currentModel?.inputs?.generate_audio?.default !== undefined) {
          params.generate_audio = currentModel.inputs.generate_audio.default;
        }
  ...
  const selectedModelBrand = getVideoModelBrand(currentModelObj || { id: selectedModel });
      ...
      <div className="absolute bottom-4 w-full max-w-[95%] lg:max-w-4xl z-40 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
        <div className="w-full bg-[#0a0a0a]/80 backdrop-blur-3xl rounded-md border border-white/10 p-4 flex flex-col gap-2 shadow-2xl">
          ...
          <ModelParameterControls
            ...
            skipKeys={VIDEO_MODEL_PARAM_SKIP_KEYS}
            className="px-1"
          />
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2 border-t border-white/[0.03] relative">
            <div className="flex items-center gap-2 relative flex-wrap pb-1 md:pb-0">
              ...
              <div className="relative">
                ...
                >
                  <VideoBrandLogo brand={selectedModelBrand} size="small" />
