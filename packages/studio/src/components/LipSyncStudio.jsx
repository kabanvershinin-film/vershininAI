...
import {
  studioVisibleLipSyncModels as lipsyncModels,
  studioVisibleImageLipSyncModels as imageLipSyncModels,
  studioVisibleVideoLipSyncModels as videoLipSyncModels,
  getLipSyncModelById,
  getResolutionsForLipSyncModel,
} from "../models.js";

const LIPSYNC_MODEL_PARAM_SKIP_KEYS = [
  "audio_url",
  "image_url",
  "video_url",
  "prompt",
...
export default function LipSyncStudio({
  ...
  const [modelParams, setModelParams] = useState(() =>
    createInitialModelParams(firstModel, {}, LIPSYNC_MODEL_PARAM_SKIP_KEYS),
  );
  ...
  useEffect(() => {
    if (selectedModel || currentModels.length === 0) return;
    const first = currentModels[0];
    setSelectedModelId(first.id);
    setSelectedResolution(first.inputs?.resolution?.default ?? "480p");
    setModelParams((prev) =>
      createInitialModelParams(first, prev, LIPSYNC_MODEL_PARAM_SKIP_KEYS),
    );
  }, [selectedModel, currentModels]);

  useEffect(() => {
    setModelParams((prev) =>
      createInitialModelParams(selectedModel, prev, LIPSYNC_MODEL_PARAM_SKIP_KEYS),
    );
  }, [selectedModel?.id]);
  ...
  const handleModelSelect = (model) => {
    ...
    setModelParams((prev) =>
      createInitialModelParams(model, prev, LIPSYNC_MODEL_PARAM_SKIP_KEYS),
    );
      ...
      <div className="absolute bottom-4 w-full max-w-[95%] lg:max-w-4xl z-40 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
        <div className="w-full bg-[#0a0a0a]/80 backdrop-blur-3xl rounded-md border border-white/10 p-4 flex flex-col gap-2 shadow-2xl">
          ...
          <ModelParameterControls
            ...
            skipKeys={LIPSYNC_MODEL_PARAM_SKIP_KEYS}
            className="px-1"
          />
