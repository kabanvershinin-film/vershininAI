...
function providerCopy(providerId, providerName, providerUrl) {
  if (providerId === 'muapi') {
    return {
      name: providerName || 'MuAPI',
      url: providerUrl || 'https://muapi.ai/access-keys',
      label: 'MuAPI API Key',
  ...
  if (providerId === 'volcengine') {
    return {
      name: providerName || 'Volcengine Ark',
      url: providerUrl || 'https://console.volcengine.com/ark',
      label: 'Volcengine Ark API Key',
      placeholder: 'Paste your Volcengine Ark key here...',
      linkText: 'Open Volcengine Ark ->',
    };
  }
  return {
    name: providerName || 'MemeFast',
    url: providerUrl || 'https://memefast.top',
    label: 'MemeFast API Key',
    placeholder: 'Paste your MemeFast key here...',
    linkText: 'Open MemeFast ->',
  };
}
...
export default function ApiKeyModal({
  ...
  const copy = providerCopy(providerId, providerName, providerUrl);
      ...
      <div className="w-full max-w-sm bg-[#0a0a0a]/90 backdrop-blur-xl border border-white/10 rounded-xl p-10 shadow-2xl relative">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <p className="text-white/40 text-[13px] leading-relaxed px-4">
            {subtitle || (
              <>Enter your <a href={copy.url} target="_blank" rel="noreferrer" className="text-[#22d3ee] hover:text-[#e5ff33] transition-colors">{copy.name}</a> API key to start creating</>
            )}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-center text-[12px] text-white/20 pt-2">
            Need a key?{' '}
            <a href={copy.url} target="_blank" rel="noreferrer" className="text-white/40 hover:text-[#22d3ee] transition-colors font-medium">
              {copy.linkText}
            </a>
          </p>
        </form>
